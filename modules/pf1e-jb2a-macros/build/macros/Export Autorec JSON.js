...
settings.ontoken = await game.settings.get(
  "autoanimations",
  "aaAutorec-ontoken"
)
settings.templatefx = await game.settings.get(
  "autoanimations",
...
for (const key of Object.keys(settings)) {
  const sorted = [...settings[key]].sort(
    (a, b) => !!b.metaData?.name - !!a.metaData?.name
  )
  ...
  sorted.map((x) => {
    ...
    x.metaData.name = x.metaData.name ?? "PF1e Animations"
    x.metaData.moduleVersion =
      x.metaData.moduleVersion ?? game.modules.get("pf1e-jb2a-macros").version
...
settings.version = await game.settings.get("autoanimations", "aaAutorec")
  .version
...
// modules/pf1e-jb2a-macros/module
