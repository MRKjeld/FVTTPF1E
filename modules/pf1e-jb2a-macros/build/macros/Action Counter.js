...
if (args[0] === "off") {
  const hook = pf1eAnimations.hooks.actionCounter.find(
    (h) => h.uuid === args[1].item.uuid
  )
  Hooks.off("preUpdateItem", hook.id)
  pf1eAnimations.hooks.actionCounter =
    pf1eAnimations.hooks.actionCounter.filter(
      (h) => h.uuid !== args[1].item.uuid
    )
  return
}
...
async function updateImages(data, changes) {
  ...
  pf1eAnimations.debug("Action Counter", {
    badgeValue,
