...
switch (args[0].item.system.slug) {
  case "dimensional-assault":
    ...
    break
  case "snare-hopping":
    opts.range = 10 * spellLevel
    opts.reverseTeleport = true
    break
  case "abundant-step":
    opts.range = Math.max(15, movementSpeed)
    break
  case "blink-charge":
    opts.range = (spellLevel - 4) * 60
    break
  case "collective-transposition":
    opts.range = 30
    opts.reverseTeleport = true
    opts.possibleTargets = 2 + (spellLevel - 6)
    break
  case "dimension-door":
  case "translocate":
    opts.range = spellLevel < 5 ? 120 : 1000
    break
  case "dimensional-steps":
    opts.range = 20 + 5 * (spellLevel - 4)
    break
  case "shadow-jump":
    opts.range = 120
    break
  case "teleport":
    opts.range = 100 * (10 * (spellLevel - 6))
    break
  case "unexpected-transposition":
    opts.range = 30
    opts.switcheroo = true
    ui.notifications.info(
      "PF1e Animations | Select a target to switch places with!"
    )
    break
...
let targetLocation = await warpgate.crosshairs.show(
  {
    size: tokenD.data.width,
    icon: tokenD.data.texture.src,
    label: "0 ft.",
...
if (cachedDistance > opts.range) {
  ui.notifications.error(
    `Your teleport has a maximum range of ${opts.range} ft. Last try.`
  )
  targetLocation = await warpgate.crosshairs.show(
    {
      size: tokenD.data.width,
      icon: tokenD.data.texture.src,
      label: "0 ft.",
...
if (cachedDistance > opts.range) {
  ui.notifications.error(
    `Your teleport has a maximum range of ${opts.range} ft.`
  )
  return
}

let targetToken = warpgate.crosshairs
...
switch (args[0].item.system.slug) {
  ...
  case "shadow-jump": {
    await Sequencer.Preloader.preloadForClients([
      "jb2a.misty_step.02.dark_black",
      "jb2a.misty_step.01.dark_black",
    ])
    seq
      .effect()
  ...
  case "terrain-transposition": {
    await Sequencer.Preloader.preloadForClients([
      "jb2a.swirling_leaves.complete.01.green.0",
      "jb2a.energy_strands.range.standard.dark_green.03",
    ])
    seq
      .effect()
