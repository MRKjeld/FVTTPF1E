...
import { applyDaeEffects, daeSystemClass, libWrapper, getToken, DAEReadyComplete, ceInterface, actionQueue, daeMacro, daeInitMacroActors, pendingInitMacroEffects } from "../dae.js";
    ...
    if (initMacrosOnLoad && !daeInitMacroActors.has(this.uuid)) {
        ...
        for (const ef of this.allApplicableEffects()) {
            if (ef.disabled || ef.isSuppressed)
                continue;
            if (ef.changes.some(c => c.key.startsWith("macro.itemMacro") || c.key.startsWith("macro.activityMacro") || c.key.startsWith("macro.execute"))) {
                initEffects.push(ef);
            }
        }
