...
import { actionQueue, actorFromUuid, addEffectChange, applyDaeEffects, atlActive, daeMacro, daeInitMacroActors, effectIsTransfer, enumerateBaseValues, getSelfTarget, getStaticID, libWrapper, localizationMap, midiActive, noDupDamageMacro, pendingInitMacroEffects, removeEffectChange, timesUpActive, useDetermineSuppression } from "../dae.js";
...
export class DAESystemDND5E extends CONFIG.DAE.systemClass {
    ...
    static initActions() {
        ...
        if (foundry.utils.isNewerVersion(game.system.version, "5.2.99")) {
            this.fieldMappings["system.traits.di.all"] = { key: "system.traits.di.value", value: "ALL", mode: CONST.ACTIVE_EFFECT_MODES.ADD };
            this.fieldMappings["system.traits.dr.all"] = { key: "system.traits.dr.value", value: "ALL", mode: CONST.ACTIVE_EFFECT_MODES.ADD };
            this.fieldMappings["system.traits.dv.all"] = { key: "system.traits.dv.value", value: "ALL", mode: CONST.ACTIVE_EFFECT_MODES.ADD };
        }
    ...
    static daeCustomEffect(actor, change, current, delta, changes) {
        ...
        switch (change.key) {
            ...
            case "system.traits.sdv.all":
                ...
                if (foundry.utils.isNewerVersion(game.system.version, "5.2.99")) {
                    foundry.utils.setProperty(actor, key, new Set(["ALL"]));
                }
                else {
...
function applyBaseEffects() {
    ...
    // CI changes get priority 0 (set in applyDaeEffects) so they sort first within the pass.
    // The CI filter on actor.statuses runs after all passes complete (in prepareData).
    ...
    try {
        ...
        if (initMacrosOnLoad && !daeInitMacroActors.has(this.uuid)) {
            ...
            for (const ef of this.allApplicableEffects()) {
                const hasMacro = ef.changes.some(c => c.key.startsWith("macro.itemMacro") || c.key.startsWith("macro.activityMacro") || c.key.startsWith("macro.execute"));
